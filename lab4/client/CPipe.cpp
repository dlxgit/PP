...
const std::string CPipe::RESULT = "\\\\.\\pipe\\result";
const std::string CPipe::READY = "\\\\.\\pipe\\ready";
const std::string CPipe::BEGIN_CALCULATION = "\\\\.\\pipe\\begin";

void CPipe::WaitMessages(std::vector<std::string> & messages, size_t nMessages, std::string const & pipeName)
{
...
void CPipe::SendMessage(std::string const & message, std::string const & pipeName)
{
	HANDLE hPipe;
	std::wstring wName(pipeName.begin(), pipeName.end());
	do
